...
1. Navigate back to IoT Hub and select **Overview** under **Security** on the left side menu.
...
1. In the **Log Analytics workspace** pane provide a unique name like **iotworkspace-<inject key="DeploymentID" enableCopy="false"/>**, keep the remaining as default, and click **OK**.
...
1. In the Essentials area near the top of the blade, to the right of **Hostname**, click **Copy to clipboard**, and then save the value in notepad for later use.

    > **Note**: The IoT Hub Hostname looks similar to: iothub-<inject key="DeploymentID" enableCopy="false"/>.azure-devices.net
...
1. Navigate to your Linux machine **linuxagentvm-<inject key="DeploymentID" enableCopy="false"/>** in Azure Portal by selecting Resource groups from the Azure home page, and then select the **linuxagentvm-<inject key="DeploymentID" enableCopy="false"/>** Linux VM resource from the list.

1. At the top of the blade, click **Connect**, and then click **SSH**.

1. Use the SSH command to create a command for connection to your VM.

   **ssh demouser@<inject key="LinuxVMIP" style="color:IndianRed"/>**
...
1. Verify that you have the **Primary key** value for your **turbine-01** IoT device is available.
   ...
   > **Note**: You should have saved the Primary key value earlier in this lab. If not, complete the following:
    ...
    1. Return the Azure Cloud Shell - you should still be connected to your **linuxagentvm-<inject key="DeploymentID" enableCopy="false"/>** virtual machine.
...
1. At the Cloud Shell command prompt and then run the following command: 
    ...
    > sudo ./InstallSecurityAgent.sh -i -aui Device -aum SymmetricKey -f /home/demouser/s.key -hn iothub-<inject key="DeploymentID" enableCopy="false"/>.azure-devices.net -di turbine-01`
      
    > **IMPORTANT**:
    > Ensure you use the full IoT Hub host name - i.e. **iothub-<inject key="DeploymentID" enableCopy="false"/>.azure-devices.net** for the `-hn` switch value.
