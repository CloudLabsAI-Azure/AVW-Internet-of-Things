...
1. If the Log Analytics workspace is not yet configured, click on the toggle button to choose the workspace and then click on **Create New Workspace** 
    
     ![](media/new_logworkspace.png)

1. In **Create Log Analytics workspace** select the existing subscription and Respurce Group and for the name of Log Analytics workspace provide as **iot-loganalytics-<inject key="DeploymentID" enableCopy="false"/>** and click **Review + Create**, once the validation is passed click **Create**
    
     ![](media/create_loganalytics.png)
    
1. Once the deployment got succeeded, refresh the portal and select the workspace you have created just now from the drop-down for **Workspace**.
...
1. At the Cloud Shell command prompt, to check the deployment status of the Azure Security Center for IoT Agent, enter the following command. 
   ...
   > **Note**: Ensure that the Cloud Shell session is still connected to your VM via SSH or else run **ssh demouser@<inject key="LinuxVMIP" style="color:IndianRed" enableCopy="true"/>**
...
1. On the left-side menu, under **Defender for IoT**, click **Overview**.
    ...
    > **Note**: It can take more than an hour for alerts to be displayed on the dashboard.

1. Once the alert is created succesfully, navigate to **Threat detection**, click the **Devices with the most alerts** tile.
