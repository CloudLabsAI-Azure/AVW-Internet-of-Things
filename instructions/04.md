...
16. Click **Review + create** and once the validation is passed click **Create** 

17. Under **Deployment**, take a minute to review the deployment manifest that is displayed. 
...
18. At the bottom of the blade, to finish setting the modules for the device, click **Create**.

19. On the **turbine-06** blade, under **Modules**, notice that **turbinesensor** is now listed.
...
20. At the top of the blade, click **Refresh**.

21. Notice that the **RUNTIME STATUS** for the **turbinesensor** module is now set to **running**.

    If the value is still not reported, wait a moment, and refresh the blade again.
 
22. Open a Cloud Shell session (if it is not still open).
...
23. At the Cloud Shell command prompt, to list the modules currently running on the IoT Edge Device, enter the following command:

    ```cmd/sh
    iotedge list
    ```

24. The output of the command looks similar to the following. 
...
<!-- TOC -->
       ...
       turbinesensor    running          Up 34 seconds    mcr.microsoft.com/azureiotedge-simulated-temperature-sensor:1.0
  
<!-- /TOC -->
  
   Notice that `turbinesensor` is listed as one of the running modules.

25. To view the module logs, enter the following command:
...
26. The Simulated Temperature Sensor Module will stop after it sends 500 messages. It can be restarted by running the following command:
...
1. On the left side navigation menu under **Configure**, click Storage account settings. Then ensure that the **iotstorage<inject key="DeploymentID" enableCopy="false"/>** storage account is added.
 
   > **Note**: If storage account is not connected follow the below steps to connect it.
     
     1. Click on **Add storage account**
 
     2. Now under storage account settings, select **Select blob storage/ADLS Gen2 from your subscriptions**.
 
     3. Select your subscription and under storage account select **iotstorage<inject key="DeploymentID" enableCopy="false"/>**, then click on **Save**.
...
2. At the command prompt, to view a list of the modules deployed to the device, enter the following command:
...
3. At the command prompt, to watch the telemetry being sent from the Edge device by the `turbinesensor` module, enter the following command:

    ```cmd/sh
    iotedge logs turbinesensor
    ```

4. Take a minute to observe the output.
...
5. While watching the temperature telemetry being sent by **turbinesensor**, notice that a **reset** command is sent by the Stream Analytics job when the `machine.temperature` reaches an average above `72`. This is the action configured in the Stream Analytics job query.
